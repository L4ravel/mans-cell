...
export default function TambahDiskonPage() {
  ...
  const stats = useMemo(() => {
    const totalDiskon = filtered.length;
    const totalAktif = filtered.filter((item) => item.isActive).length;
    const totalBarangPromo = filtered.reduce(
      (sum, item) => sum + item.barangIds.length,
      0,
    );
    const totalTokoPromo = new Set(
      filtered.map((item) => item.tokoId).filter(Boolean),
    ).size;

    return { totalDiskon, totalAktif, totalBarangPromo, totalTokoPromo };
  }, [filtered]);
  ...
  const getHargaSebelumText = (items: DiskonBarangRingkas[]) => {
    ...
    const hargaList = items
      .map((item) => Number(item.hargaJual || 0))
      .sort((a, b) => a - b);
  ...
  const validateForm = () => {
    ...
    const duplicate = data.find((item) => {
      const sameName =
        item.namaPromo.trim().toLowerCase() === namaPromo.toLowerCase();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const barangDipilih = barangList.filter(
        (item) =>
          item.tokoId === form.tokoId && form.barangIds.includes(item.id),
      );
      ...
      if (isEdit && editId) {
        ...
        setData((prev) =>
          [...prev, newItem].sort((a, b) =>
            a.namaPromo.localeCompare(b.namaPromo),
          ),
        );
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold leading-relaxed text-sky-50/85 sm:text-sm">
                  Kelola promo diskon berdasarkan toko dan barang agar otomatis
                  terbaca di transaksi kasir.
                </p>
              </div>
            </div>
            ...
            <div className="hidden flex-wrap items-center gap-2 sm:flex">
              ...
              <motion.button
                ...
                type="button"
              >
                <Package size={12} strokeWidth={2.8} />
                <span>Barang</span>
              </motion.button>
          ...
          {successMsg && (
            ...
            >
              <CheckCircle2
                ...
                strokeWidth={2.5}
              />
              <p className="max-w-xs text-xs font-black text-sky-700">
                {successMsg}
              </p>
            </motion.div>
          )}
                  ...
                  <div className="space-y-5 p-6">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                        ...
                        <div className="w-full sm:max-w-xs">
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setSearchBarangModal(e.target.value)
                              }
                      ...
                      {form.barangIds.length > 0 && (
                        <p className="mt-3 text-[11px] font-bold uppercase tracking-wide text-sky-600">
