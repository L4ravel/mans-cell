// Halaman admin barang tetap untuk CRUD data barang tetap ke Firestore.
// Revisi ini dibuat konsisten dengan halaman barang jual, tapi khusus aset/barang tidak dijual dengan 1 field harga saja.
...
export default function TambahBarangTetapPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && editId) {
        const updatedItem: Partial<BarangTetap> = {
          nama,
        ...
        await updateDoc(doc(db, COLLECTION_NAME, editId), {
          ...updatedItem,
          updatedBy: user.uid,
        })
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 sm:h-14 sm:w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 to-cyan-500 shadow-lg shadow-emerald-200/50">
              <Building2 size={24} className="text-white sm:w-7 sm:h-7" strokeWidth={2.5} />
            </div>
            <div>
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-700">
                    Rp {Number(d.harga || 0).toLocaleString("id-ID")}
                  </p>
                </div>
              </div>
            </motion.div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-white/80 border-b border-slate-200">
                <tr>
                  {["No", "Nama", "Toko", "Kategori", "Merk", "Harga Barang", "Aksi"].map((h) => (
                    <th
                      key={h}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="rounded-xl border border-cyan-100 bg-cyan-50/60 px-3 py-2.5">
                      ...
                      <p className="text-sm font-bold text-cyan-700 mt-1">
                        Rp {Number(form.harga || 0).toLocaleString("id-ID")}
                      </p>
                    </div>
            ...
            >
              <div className="px-6 py-4 bg-gradient-to-r from-red-500 to-rose-500">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-base font-black text-white">Hapus Barang Tetap</h2>
                </div>
              </div>
