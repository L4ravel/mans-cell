// Halaman admin kategori barang untuk CRUD data kategori langsung ke Firestore dari client.
// Layout dibuat konsisten seperti page tambah karyawan, dengan update local state tanpa reload semua data agar hemat read Firestore.
  ...
  return (
    <div>
      ...
      <select
        ...
        className="w-full appearance-none rounded-xl border-2 border-slate-200 bg-white pl-3 pr-3 py-2.5 text-sm font-semibold text-slate-700 transition-all hover:border-cyan-300 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20"
      >
        {children}
      </select>
    </div>
...
export default function KategoriBarangPage() {
  ...
  const openEdit = (d: KategoriBarang) => {
    setForm({
      nama: d.nama,
    })
    setEditId(d.id)
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "kategori_barang", deleteId))
  ...
  <div className="flex min-w-0 items-center gap-3 sm:items-start sm:gap-4">
    <div className="flex h-12 w-12 sm:h-14 sm:w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 to-cyan-500 shadow-lg shadow-emerald-200/50">
      <Tag size={22} className="text-white sm:w-7 sm:h-7" strokeWidth={2.5} />
    </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-white/70 font-semibold mt-0.5">
                      {isEdit ? "Perbarui kategori barang" : "Isi field wajib (*)"}
                    </p>
                  </div>
                </div>
            ...
            >
              ...
              <div className="px-6 py-5">
                <p className="text-sm text-slate-600 font-semibold">
                  Yakin ingin menghapus kategori ini? Tindakan ini{" "}
