/* 
  Halaman admin satuan barang untuk CRUD data satuan langsung ke Firestore dari client.
...
export default function SatuanBarangPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const list: SatuanBarang[] = snap.docs.map((d) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && editId) {
        ...
        setSuccessMsg("Satuan berhasil diperbarui")
      } else {
        ...
        setSuccessMsg("Satuan berhasil ditambahkan")
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "satuan_barang", deleteId))

      setData((prev) => prev.filter((item) => item.id !== deleteId))
      setDeleteId(null)
      setSuccessMsg("Satuan berhasil dihapus")
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 to-cyan-500 shadow-lg shadow-emerald-200/50 sm:h-14 sm:w-14">
              <Ruler size={24} className="text-white sm:h-7 sm:w-7" strokeWidth={2.5} />
            </div>
            <div>
              <h1 className="leading-none tracking-tight text-slate-800 text-xl font-black sm:text-2xl">
                Satuan Barang
              </h1>
              <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">
                Satuan produk · master data
              </p>
            </div>
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] font-semibold text-white/70">
                      {isEdit ? "Perbarui satuan barang" : "Isi field wajib (*)"}
                    </p>
                  </div>
                </div>
            ...
            >
              <div className="bg-gradient-to-r from-red-500 to-rose-500 px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-base font-black text-white">Hapus Satuan</h2>
                </div>
              </div>

              <div className="px-6 py-5">
                <p className="text-sm font-semibold text-slate-600">
                  Yakin ingin menghapus satuan ini? Tindakan ini{" "}
