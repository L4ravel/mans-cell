// Halaman admin provider untuk CRUD data provider langsung ke Firestore dari client.
// Layout dibuat konsisten seperti page tambah kategori, dengan update local state tanpa reload semua data agar hemat read Firestore.
...
export default function ProviderPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const list: ProviderItem[] = snap.docs.map((d) => {
  ...
  const validateForm = () => {
    if (!form.nama.trim()) return "Nama provider wajib diisi"

    const namaNormalized = form.nama.trim().toLowerCase()
    const isDuplicate = data.some(
      (item) =>
        item.nama.trim().toLowerCase() === namaNormalized &&
        item.id !== editId
    )

    if (isDuplicate) return "Nama provider sudah ada"
    return null
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEdit && editId) {
        const updatedItem: Partial<ProviderItem> = {
          nama,
          updatedAt: Date.now(),
        }
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "provider", deleteId))
  ...
  return (
    <div className="space-y-4 text-slate-900 sm:space-y-5">
      <motion.div
        ...
        className="relative overflow-hidden rounded-xl border-b border-r border-t border-l-4 border-l-emerald-500 border-slate-200 bg-white p-4 shadow-sm sm:p-5"
      >
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
          <div className="flex min-w-0 items-center gap-3 sm:items-start sm:gap-4">
            <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 to-cyan-500 shadow-lg shadow-emerald-200/50 sm:h-14 sm:w-14">
              <Wifi size={22} className="text-white sm:h-7 sm:w-7" strokeWidth={2.5} />
            </div>
      ...
      <motion.div
        ...
        className="rounded-xl border-b border-r border-t border-l-4 border-l-blue-500 border-slate-200 bg-white p-4 shadow-sm"
      >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border-2 border-slate-200 bg-white py-2.5 pl-8 pr-3 text-sm font-semibold text-slate-700 placeholder:text-slate-300 placeholder:font-normal transition-all hover:border-cyan-300 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/20"
              />
            </div>
          </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] font-semibold text-white/70">
                      {isEdit ? "Perbarui provider" : "Isi field wajib (*)"}
                    </p>
                  </div>
                </div>
                  ...
                  >
                    {submitLoading ? (
                      ...
                      <>
                        ...
                        {isEdit ? "Perbarui" : "Simpan Provider"}
                      </>
                    )}
                  </motion.button>
                </div>
            ...
            >
              <div className="bg-gradient-to-r from-red-500 to-rose-500 px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-base font-black text-white">Hapus Provider</h2>
                </div>
              </div>

              <div className="px-6 py-5">
                <p className="text-sm font-semibold text-slate-600">
                  Yakin ingin menghapus provider ini? Tindakan ini{" "}
