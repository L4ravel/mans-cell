/*
  ...
  CRUD metode pembayaran Firestore, layout biru muda konsisten dengan master data terbaru,
  filter mobile collapse, card mobile, tabel desktop, toast fixed, dan update local state tanpa reload penuh.
*/
...
export default function TambahMetodePembayaranPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const list: MetodePembayaran[] = snap.docs.map((item) => {
        const x = item.data() as any

        return {
          id: item.id,
          nama: normalizeText(x?.nama),
          tipe: x?.tipe === "Non-Tunai" ? "Non-Tunai" : "Tunai",
          provider: normalizeText(x?.provider),
          biayaAdmin: Number(x?.biayaAdmin || 0),
          nomorRekening: normalizeText(x?.nomorRekening),
          namaRekening: normalizeText(x?.namaRekening),
          aktif: x?.aktif !== false,
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((item) => {
      const matchSearch =
        ...
        String(item.nomorRekening || "").toLowerCase().includes(q) ||
        String(item.namaRekening || "").toLowerCase().includes(q) ||
  ...
  }, [data, search, filterTipe, filterStatus])

  const stats = useMemo(() => {
    const totalMetode = data.length
    const totalAktif = data.filter((item) => item.aktif).length
    const totalNonTunai = data.filter((item) => item.tipe === "Non-Tunai").length
    const totalDenganAdmin = data.filter((item) => Number(item.biayaAdmin || 0) > 0).length
  ...
  const openEdit = (item: MetodePembayaran) => {
    setEditId(item.id)
    setForm({
      nama: item.nama || "",
      tipe: item.tipe || "Tunai",
      provider: item.provider || "",
      biayaAdmin: String(item.biayaAdmin || 0),
  ...
  const validateForm = () => {
    ...
    if (!nama) return "Nama metode pembayaran wajib diisi"
    if (!form.tipe) return "Tipe pembayaran wajib dipilih"
    if (Number.isNaN(biayaAdmin) || biayaAdmin < 0) return "Biaya admin tidak valid"
    if (biayaAdmin > 100) return "Biaya admin tidak boleh lebih dari 100%"
    ...
    if (form.tipe === "Non-Tunai" && !form.provider.trim()) return "Provider wajib diisi untuk metode non-tunai"
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, "metode_pembayaran", deleteTarget.id))
      setData((prev) => prev.filter((item) => item.id !== deleteTarget.id))
      setDeleteTarget(null)
      showSuccess("Metode pembayaran berhasil dihapus")
    } catch (e) {
      console.error(e)
      showError("Gagal menghapus metode pembayaran")
    } finally {
      ...
      <main className="relative w-full space-y-4 pb-28">
            ...
            <div className="flex min-w-0 items-start gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs font-semibold leading-relaxed text-sky-50/85 sm:text-sm">
                  Kelola tunai, transfer, QRIS, e-wallet, biaya admin, dan status pembayaran.
                </p>
              </div>
            </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4">
            <MetodeStatCard label="Metode" value={String(stats.totalMetode)} icon={Wallet} tone="sky" />
            <MetodeStatCard label="Aktif" value={String(stats.totalAktif)} icon={ShieldCheck} tone="blue" />
            <MetodeStatCard label="Non Tunai" value={String(stats.totalNonTunai)} icon={CreditCard} tone="slate" />
            <MetodeStatCard label="Admin" value={String(stats.totalDenganAdmin)} icon={BadgePercent} tone="rose" />
          </div>
        </div>
              ...
              <FieldBox label="Cari Metode">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Nama, provider, rekening, atau tipe..."
...
function MetodeSection({
  ...
  loading: boolean
  filtered: MetodePembayaran[]
  paged: MetodePembayaran[]
  page: number
  totalPages: number
  itemsPerPage: number
  goPage: (page: number) => void
  openAdd: () => void
  openEdit: (item: MetodePembayaran) => void
  setDeleteTarget: (item: MetodePembayaran) => void
}) {
  if (loading) {
    return (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 truncate text-[10px] font-black uppercase tracking-[0.12em] text-slate-400">
                      {item.tipe} · {item.provider || "Tanpa Provider"}
                    </p>
                  </div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 border-t border-slate-100 pt-3">
                  <InfoMini label="Provider" value={item.provider || "—"} />
                  <InfoMini label="Admin" value={`${item.biayaAdmin || 0}%`} />
                  <InfoMini label="No. Rekening" value={item.nomorRekening || "—"} />
                  <InfoMini label="Nama Rekening" value={item.namaRekening || "—"} />
                </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead className="border-b border-slate-100 bg-slate-50/70">
              <tr>
                {["No", "Metode", "Tipe", "Provider", "Admin", "No. Rekening", "Nama Rekening", "Status", "Update", "Aksi"].map((head) => (
                  <th
                    key={head}
              ...
              {paged.map((item, index) => (
                <tr key={item.id} className="border-t border-slate-100 transition-colors hover:bg-sky-50/40">
                  ...
                  <td className="whitespace-nowrap px-3 py-3">
                    <span className="rounded-lg bg-sky-50 px-2 py-1 font-black text-sky-700">
                      {item.tipe}
                    </span>
                  </td>
                  <td className="whitespace-nowrap px-3 py-3 font-semibold text-slate-600">{item.provider || "—"}</td>
                  <td className="whitespace-nowrap px-3 py-3 font-semibold text-slate-600">{item.biayaAdmin || 0}%</td>
                  <td className="whitespace-nowrap px-3 py-3 font-semibold text-slate-600">{item.nomorRekening || "—"}</td>
                  <td className="whitespace-nowrap px-3 py-3 font-semibold text-slate-600">{item.namaRekening || "—"}</td>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 sm:gap-3">
                  ...
                  <FieldSelect
                    ...
                    value={form.tipe}
                    onChange={(value) => setForm((prev) => ({ ...prev, tipe: value as FormState["tipe"] }))}
                    icon={CreditCard}
                  >
                    {TIPE_OPTIONS.map((item) => (
                  ...
                  <FieldInput
                    ...
                    onChange={(value) => setForm((prev) => ({ ...prev, biayaAdmin: value.replace(/[^\d.]/g, "") }))}
                    icon={BadgePercent}
                    inputMode="decimal"
                    placeholder="0"
                  />
                  ...
                  <FieldInput
                    ...
                    onChange={(value) => setForm((prev) => ({ ...prev, nomorRekening: value }))}
                  ...
                  <FieldInput
                    ...
                    onChange={(value) => setForm((prev) => ({ ...prev, namaRekening: value }))}
                    icon={Landmark}
                    placeholder="Opsional"
                  />
                  ...
                  <div className="rounded-xl border border-sky-100 bg-sky-50/70 px-3 py-2.5">
                    <p className="text-[10px] font-black uppercase tracking-widest text-sky-600">Preview</p>
                    <p className="mt-1 truncate text-xs font-black text-sky-700">
                      {form.tipe} · Admin {Number(form.biayaAdmin || 0)}% · {form.aktif ? "Aktif" : "Nonaktif"}
                    </p>
                  </div>
                </div>
          ...
          >
            <div className="relative overflow-hidden bg-gradient-to-br from-rose-500 to-red-600 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-black leading-none tracking-tight text-white">Hapus Metode</h2>
            ...
            <div className="space-y-3 p-5">
              <p className="text-[11px] font-semibold text-slate-600">
                Anda yakin mau menghapus metode pembayaran ini?
              </p>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-xs font-black text-slate-800">{target.nama}</p>
                <p className="mt-0.5 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                  {target.tipe} · {target.provider || "Tanpa Provider"}
                </p>
              </div>
            </div>
