/*
  Halaman ini dipakai untuk absensi harian karyawan berbasis GPS, lokasi toko dinamis, dan jadwal dinamis.
  Mendukung animasi no-absen untuk karyawan non wajib serta animasi waktu saat absensi belum buka/sudah tutup/libur.
*/
...
export default function AbsensiKaryawanPage() {
  ...
  const loadPengaturanJamAbsensi = useCallback(
    async (currentKaryawanId: string, currentTokoId: string, currentTanggal: string) => {
      ...
      try {
        const individuRef = doc(db, "pengaturan_jam_absensi", `karyawan_${currentKaryawanId}`)
        const tokoRef = doc(db, "pengaturan_jam_absensi", `toko_${currentTokoId}`)
        ...
        if (currentTokoId) {
          ...
          if (tokoSnap.exists()) {
            applyResolvedSchedule(tokoSnap.data(), "toko", currentTanggal)
            setJadwalLoading(false)
            return
          }
        }
  ...
  useEffect(() => {
    ...
    const unsubAuth = auth.onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        await loadPengaturanJamAbsensi(karyawanIdValue, tokoIdValue, tanggal)
      ...
      <div className="relative min-h-screen bg-[#f8fafc] text-slate-900">
        ...
        <div className="relative z-10 mx-auto w-full px-3 py-3 sm:px-4 sm:py-4 lg:p-8 max-w-md sm:max-w-xl md:max-w-3xl lg:max-w-5xl">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
            ...
            {isTidakWajib ? (
              <div className="fixed inset-0 flex items-center justify-center bg-[#f8fafc] z-20">
                <AnimasiNoAbsen />
              </div>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 pb-20">
                  ...
                  <motion.button
                    whileHover={
                      !(!sudahMasuk || sudahPulang || sudahIzinAtauSakit)
                        ? { scale: 1.02 }
                        : {}
                    }
                    whileTap={
              ...
              <div className="fixed inset-0 flex items-center justify-center bg-[#f8fafc] z-20">
                <AnimasiWaktu
                  ...
                  hariLibur={jadwalAbsensi.isLibur ? [new Date(tanggal).getDay()] : []}
                />
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="pt-2">
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={closeAlert}
                        className={`w-full px-5 py-3 rounded-full font-black uppercase tracking-[0.1em] text-white shadow-lg transition-all text-[11px] ${
                          alertModal.type === "success"
                            ...
                            : alertModal.type === "warning"
                            ? "bg-gradient-to-r from-yellow-400 to-orange-500 shadow-yellow-200/50 hover:shadow-xl"
