/* 
  Layout ini menjadi gate auth dan shell dashboard untuk halaman pelanggan.
  Sidebar, dropdown user, logout, dan ganti password sudah disiapkan.
  Menu pelanggan masih dummy dulu sampai halaman aslinya dibuat.
*/
...
const MENU_ITEMS = [
  { href: "/pelanggan", icon: Home, label: "Beranda" },
  { href: "/pelanggan/member", icon: CreditCard, label: "Member Saya" },
  { href: "/pelanggan/riwayat", icon: ReceiptText, label: "Riwayat Belanja" },
  { href: "/pelanggan/profil", icon: UserRound, label: "Profil Saya" },
]
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        const raw = snap.data()
        setUserName(raw?.nama || "Pelanggan")
        ...
        console.error("Gagal validasi user pelanggan:", error)
        router.replace("/unauthorized")
      }
    })
